...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  try {
    $stmt = $pdo->prepare("SELECT id FROM userinformation WHERE email = ?");
    $stmt->execute([$email]);
    ...
    $message = "Error: " . $foutje->getMessage();
  }
}
}
?>
...
<body class="overflow-x-hidden">
  <header>
    <nav
      ...
      <div class="lg:ml-auto">
        <a class="bg-[#EEEEEE] hover:bg-[#CCCCCC] transition rounded-full px-4 py-2 opensans" href="login.php">Login</a>
      </div>
    </nav>
  </header>
  <main class="flex items-center justify-center mt-24 mb-24 px-4">
    <div class="bg-[#EEEEEE] opacity-80 rounded-3xl p-8 w-full max-w-md">
      <h1 class="montserrat text-3xl font-bold text-center mb-6">
        Create Account
      </h1>

      <form method="POST" class="flex flex-col gap-4">
        <div class="flex flex-col gap-2">
          <label class="opensans" for="name">Full Name</label>
        ...
        <div class="flex flex-col gap-2">
          <label class="opensans" for="confirm">Confirm Password</label>
        ...
        <?php if (isset($message)) echo "<p class='text-black text-center mt-2'>$message</p>"; ?>
